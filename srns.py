...
class PhiScene(nn.Module):
    def __init__(self):
        super(PhiScene, self).__init__()
        ...
        layers.pop(0)
        layers.insert(0, nn.Conv2d(3, 64, kernel_size=(7, 7), stride=(2, 2), padding=(3, 3), bias=False))

        self.main = nn.Sequential(*layers)
        self.final_layer = nn.Linear(512, 3)

    def forward(self, img_):
        feature_map_ = self.main(img_)
        vec_ = self.final_layer(feature_map_.squeeze())
        return vec_

class DepthCompute(nn.Module):
    def __init__(self, in_c=5):
        super(DepthCompute, self).__init__()
        self.final_layer = nn.Sequential(
            nn.Linear(in_c, in_c*2),
            nn.LeakyReLU(0.01),
            nn.Linear(in_c*2, in_c*2),
            nn.LeakyReLU(0.01),
    ...
    def forward(self, vec_):
        vec_out_ = self.final_layer(vec_)*40-20
        return vec_out_

class SRNsModel2(nn.Module):
    def __init__(self,
                 num_instances,
                 latent_dim,
                 tracing_steps,
        ...
        self.ray_marcher = custom_layers.Raymarcher2(num_feature_channels=self.num_hidden_units_phi,
                                                     raymarch_steps=self.sphere_trace_steps)
...
class SRNsModel3(nn.Module):
    def __init__(self,
        ...
        self.phi2 = pytorch_prototyping.FCBlock(hidden_ch=self.num_hidden_units_phi,
                                                num_hidden_layers=self.phi_layers - 2,
                                                in_features=3,
                                                out_features=self.num_hidden_units_phi)
    ...
    def get_image_loss(self, prediction, ground_truth):
        trgt_imgs = ground_truth['rgb']
        trgt_imgs = trgt_imgs.cuda()
        loss = self.l2_loss(prediction, trgt_imgs)
        return loss
    ...
    def forward(self, input, z=None):
        ...
        intrinsics = None
        uv = uvs.cuda().float()
        v_scene = self.phi1(rgb_mat)
        v_scene = expand(v_scene, uv.size(1))
        depth = self.ray_marcher(torch.cat([uv, v_scene], dim=2))

        points_xyz = geometry.world_from_xy_depth(uv,
                                                  depth,
                                                  intrinsics=intrinsics,
                                                  cam2world=pose)

        # Sample phi a last time at the final ray-marched world coordinates.
        v = self.phi2(points_xyz)
...
def expand(mat, times):
    res = torch.zeros(mat.size(0), times, mat.size(1)).to(mat.device)
...
if __name__ == '__main__':
    phi = PhiScene()
    pred = phi(torch.zeros(3, 3, 128, 128))
    print(pred.size())